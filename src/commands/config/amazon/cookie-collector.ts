...
const LOGIN_INDICATOR_SELECTOR = "#nav-link-accountList-nav-line-1"
...
async function waitForUserLogin(page: import("puppeteer").Page): Promise<void> {
    logInfo("Faça login na Amazon. Aguardando detecção (timeout: 5 min)...")
    ...
    throw new Error("Timeout aguardando login na Amazon (5 min).")
}

export async function collectCookiesViaBrowser(): Promise<Cookie[]> {
    logInfo("Abrindo navegador para login na Amazon...")
...
export function collectCookiesViaExtension(): Cookie[] {
  ...
  ${gray}Passos:${reset}
    1. Acesse ${cyan}amazon.com.br${reset} e faça login
